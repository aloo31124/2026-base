...
export default function EmailVerificationPage() {
  ...
  async function sendVerificationMail(event: FormEvent) {
    ...
    try {
      const result = await api<SendMailResult>(
    ...
    <AppShell>
      <div className="content mail-verification-content">
        ...
        <section className="card mail-verification-card">
          ...
          <form
            className="mail-verification-form"
          ...
          {logs.length === 0 ? (
            ...
            <div className="mail-log-table-wrap">
              <table>
                ...
                <tbody>
                  {logs.map((log) => (
                    <tr key={log.id} data-testid="mail-log-row">
                      <td data-label="收件者">{log.maskedRecipient}</td>
                      <td data-label="用途">{purposeLabel(log.purpose)}</td>
                      <td data-label="狀態">
                        <span
                          className={`tag ${log.status === "SUCCESS" ? "managed" : "failed"}`}
                        >
                          {log.status === "SUCCESS" ? "成功" : "失敗"}
                        </span>
                      </td>
                      <td data-label="時間">
                        {new Date(log.completedAt).toLocaleString("zh-TW")}
                      </td>
