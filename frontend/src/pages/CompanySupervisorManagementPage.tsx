...
export default function CompanySupervisorManagementPage() {
  ...
  const [employeeBindingSearch, setEmployeeBindingSearch] = useState({
    companyName: "",
    employeeName: "",
  });
  ...
  const [employeeBindingForm, setEmployeeBindingForm] =
    useState(blankEmployeeBinding);
  ...
  async function loadAll() {
    ...
    try {
      const [
        ...
        api<EmployeeBinding[]>(
          "/admin/company-supervisor-management/employee-bindings",
        ),
      ...
      setBoundEmployeeUserIds(
        employeeBindingRows.map((binding) => binding.userId),
      );
      setUsers(userRows);
    } catch (reason) {
  ...
  async function deleteCompany(id: string) {
    await mutate(
      () =>
        api<void>(`/admin/company-supervisor-management/companies/${id}`, {
          method: "DELETE",
        }),
      "公司已刪除。",
    );
  ...
  async function deleteSupervisor(id: string) {
    await mutate(
      () =>
        api<void>(`/admin/company-supervisor-management/supervisors/${id}`, {
          method: "DELETE",
        }),
      "主管已刪除。",
    );
  ...
  async function searchBindings(event: FormEvent) {
    ...
    try {
      ...
      setBindings(
        await api<Binding[]>(
          `/admin/company-supervisor-management/bindings?${query}`,
        ),
      );
    } catch (reason) {
      setError((reason as Error).message);
  ...
  async function deleteBinding(id: string) {
    await mutate(
      () =>
        api<void>(`/admin/company-supervisor-management/bindings/${id}`, {
          method: "DELETE",
        }),
      "公司主管綁定已取消。",
    );
      ...
      <div className="content company-supervisor-content">
        <header className="page-heading">
          <div>
            ...
            <h1>公司主管管理</h1>
            <p>
              維護公司主檔、既有使用者的主管身分，以及一人一公司的綁定關係。
            </p>
          </div>
          <button
        ...
        <div
          className="tabs management-tabs"
          role="tablist"
          aria-label="公司主管管理標籤"
        >
          <button
          ...
          <button
            data-testid="supervisor-tab"
            className={tab === "supervisors" ? "active" : ""}
          ...
          <section className="management-grid">
            <form className="card management-form" onSubmit={saveCompany}>
              <div>
                ...
                <h2>{companyForm.id ? "修改公司" : "新增公司"}</h2>
              </div>
              <label>
                公司名稱
                <input
              ...
              <div className="actions">
                {companyForm.id && (
                  <button
                    className="btn secondary"
                    type="button"
                    onClick={() => setCompanyForm(blankCompany)}
                  >
                    取消
                  </button>
                )}
            ...
            <div className="card management-list">
                ...
                <tbody>
                  {companies.map((company) => (
                    <tr key={company.id} data-testid="company-row">
                      ...
                      <td data-label="說明">{company.description || "—"}</td>
                      <td data-label="操作">
              ...
              {companies.length === 0 && (
                <p className="empty-state">尚無符合條件的公司。</p>
              )}
            </div>
          </section>
        )}
        ...
        {tab === "supervisors" && (
          <section className="management-grid">
            <form className="card management-form" onSubmit={saveSupervisor}>
              <div>
                <p className="eyebrow">Supervisor profile</p>
                <h2>{supervisorForm.id ? "修改主管" : "新增主管"}</h2>
              </div>
              <label>
                已註冊使用者
                <select
                  data-testid="supervisor-user"
                  required
                  disabled={Boolean(supervisorForm.id)}
                ...
                >
                  ...
                  {supervisorForm.id && (
                    <option value={supervisorForm.userId}>
                      {
                        supervisors.find((row) => row.id === supervisorForm.id)
                          ?.fullName
                      }
                    </option>
                  )}
              ...
              <label>
                主管職稱
                <input
                  data-testid="supervisor-title"
                  required
                  maxLength={80}
                  value={supervisorForm.title}
                  onChange={(event) =>
                    setSupervisorForm({
                      ...supervisorForm,
              ...
              <div className="actions">
                {supervisorForm.id && (
                  <button
                    className="btn secondary"
                    type="button"
                    onClick={() => setSupervisorForm(blankSupervisor)}
                  >
                    取消
                  </button>
                )}
            ...
            <div className="card management-list">
              ...
              <table>
                ...
                <tbody>
                  {supervisors.map((supervisor) => (
                    <tr key={supervisor.id} data-testid="supervisor-row">
                      <td data-label="主管">
                      ...
                      <td data-label="職稱">
                        <span className="tag">{supervisor.title}</span>
                      </td>
                      <td data-label="公司">
                        {supervisor.companyName || "尚未綁定"}
                      </td>
                      <td data-label="操作">
                        <div className="table-actions">
                          <ActionIconButton
                          ...
                          <ActionIconButton
                            ...
                            icon="×"
                            tone="danger"
                            disabled={Boolean(supervisor.companyId)}
              ...
              {supervisors.length === 0 && (
                <p className="empty-state">尚無符合條件的主管。</p>
              )}
            </div>
          </section>
        )}
        ...
        {tab === "bindings" && (
          <section className="binding-section">
            ...
            {bindingKind === "supervisor" ? (
              <div className="management-grid">
                <form className="card management-form" onSubmit={saveBinding}>
                  <div>
                    <p className="eyebrow">Supervisor membership</p>
                    <h2>主管綁定公司</h2>
                  </div>
                  <label>
                    公司
                    <select
                      data-testid="binding-company"
                      required
                      value={bindingForm.companyId}
                      onChange={(event) =>
                        setBindingForm({
                          ...bindingForm,
                  ...
                  <label>
                    未綁定主管
                    <select
                      data-testid="binding-supervisor"
                      required
                      value={bindingForm.supervisorId}
                      onChange={(event) =>
                        setBindingForm({
                          ...bindingForm,
                ...
                <div className="card management-list">
                  <form className="binding-search" onSubmit={searchBindings}>
                    <input
                      data-testid="binding-company-search"
                      aria-label="綁定公司查詢"
                      placeholder="公司名稱"
                      value={bindingSearch.companyName}
                      onChange={(event) =>
                        setBindingSearch({
                          ...bindingSearch,
                    ...
                    />
                    <input
                      data-testid="binding-supervisor-search"
                      aria-label="綁定主管查詢"
                      placeholder="主管姓名或帳號"
                      value={bindingSearch.supervisorName}
                      onChange={(event) =>
                        setBindingSearch({
                          ...bindingSearch,
                    ...
                    />
                    <button
                      data-testid="binding-search-submit"
                      className="btn secondary"
                    >
                      查詢
                    </button>
                  ...
                  <table>
                    ...
                    <tbody>
                      {bindings.map((binding) => (
                        <tr key={binding.id} data-testid="binding-row">
                          <td data-label="公司">
                            <strong>{binding.companyName}</strong>
                          </td>
                          <td data-label="主管">
                            {binding.supervisorName}
                            <small>{binding.supervisorUsername}</small>
                          </td>
                          <td data-label="職稱">
                            <span className="tag managed">{binding.title}</span>
                          </td>
                          <td data-label="操作">
                            <div className="table-actions">
                              <ActionIconButton
                  ...
                  {bindings.length === 0 && (
                    <p className="empty-state">尚無符合條件的主管綁定。</p>
                  )}
                </div>
              </div>
            ) : (
              <div className="management-grid">
                <form
                  className="card management-form"
                  onSubmit={saveEmployeeBinding}
                >
                  <div>
                    <p className="eyebrow">Employee membership</p>
                    <h2>員工綁定公司</h2>
                  </div>
                  <label>
                    公司
                  ...
                  <p className="form-hint">
                    僅顯示啟用、具有員工角色、尚未成為主管且未綁定公司的使用者。
                  </p>
                  <div className="actions">
                    <button
                      data-testid="employee-binding-save"
                      className="btn primary"
                      disabled={loading}
                    >
                      建立綁定
                ...
                <div className="card management-list">
                  <form
                    className="binding-search"
                    onSubmit={searchEmployeeBindings}
                  >
                    <input
                    ...
                    />
                    <button
                      data-testid="employee-binding-search-submit"
                      className="btn secondary"
                    >
                      查詢
                    </button>
                  ...
                  <table>
                    ...
                    <tbody>
                      {employeeBindings.map((binding) => (
                        <tr key={binding.id} data-testid="employee-binding-row">
                          <td data-label="公司">
                            <strong>{binding.companyName}</strong>
                          </td>
                          <td data-label="員工">
                            {binding.employeeName}
                            <small>{binding.employeeUsername}</small>
                          </td>
                          ...
                          <td data-label="操作">
                            <div className="table-actions">
                              <ActionIconButton
                                ...
                                tone="danger"
                                onClick={() =>
                                  void deleteEmployeeBinding(binding.id)
                                }
                              />
                            </div>
                          </td>
                  ...
                  {employeeBindings.length === 0 && (
                    <p className="empty-state">尚無符合條件的員工綁定。</p>
                  )}
                </div>
              </div>
            )}
        ...
        <div className="management-status" aria-live="polite">
          {message && (
            <p data-testid="company-supervisor-success" className="success">
              {message}
            </p>
          )}
          {error && (
            <p data-testid="company-supervisor-error" className="error">
              {error}
            </p>
          )}
        </div>
