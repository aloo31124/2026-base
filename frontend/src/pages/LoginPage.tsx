...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (login.fulfilled.match(result)) {
      navigate(
        result.payload.roles.includes("SYSTEM_ADMIN")
          ? "/users"
          : "/task-assignment",
      );
    }
  ...
  async function lineLogin() {
    ...
    if (requestLineAuthorization.fulfilled.match(result))
    ...
    <main className="auth-page">
      <section className="auth-visual">
        ...
        <div>
          ...
          <h1>讓權限清楚，讓工作流保持流動。</h1>
          <p>以安全驗證與角色授權保護每一個管理操作。</p>
          <div className="workflow-preview">
            <span className="workflow-node">身分驗證</span>
            <i className="workflow-line" />
            <span className="workflow-node">角色授權</span>
            <i className="workflow-line" />
            <span className="workflow-node">安全工作</span>
          </div>
        </div>
      ...
      <section className="auth-panel">
        <div className="auth-box">
          ...
          <button
            ...
            onClick={() => void lineLogin()}
          >
            <span className="line-mark" aria-hidden="true">
              LINE
            </span>
            <span>
              {status === "loading" ? "正在前往 LINE…" : "使用 LINE 登入"}
            </span>
          </button>
          ...
          <div className="auth-links">
            <Link data-testid="email-register-link" to="/register">
              使用信箱註冊
            </Link>
